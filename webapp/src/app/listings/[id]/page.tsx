  ...
  const handleContactProvider = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/messages', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          recipientId: listing.profile.id,
      ...
      if (response.ok) {
        ...
        // Fetch conversations to find the one we just created
        ...
        if (conversationsResponse.ok) {
          ...
          const conversation = conversationsData.conversations.find(
            (conv: { otherParticipant?: { id: string } }) =>
              conv.otherParticipant?.id === listing.profile.id
          );
            ...
            >
              ...
              <button
                ...
                style={{
                  width: '100%',
                  padding: '12px',
                  backgroundColor: contacting ? '#9CA3AF' : '#C86A50',
                  color: '#F7F5F2',
                  border: 'none',
                  borderRadius: '8px',
                  fontSize: '16px',
